...
from dataloader import DataLoaderGenerator, AnchorBox # Ensure AnchorBox is imported
import time
...
NUM_CLASSES = 14
NUM_ANCHORS = 9
EXPECTED_CHANNELS_CLS = NUM_ANCHORS * NUM_CLASSES
BATCH_SIZE = 25        # Test batch size
INPUT_SHAPE = (320, 320)
CONF_THRESHOLD = 0.5  # Only count detections with confidence > 50%
IOU_THRESHOLD = 0.5   # Standard threshold for mAP matching
NMS_THRESHOLD = 0.5   # Threshold for Non-Max Suppression (removing duplicates)

TEST_DATA_ROOT = "../datasets/nuscenes_preprocessed"
WEIGHTS_PATH = "./fl_weights_temp/global_weights.weights.h5"
META_TEST_DIR = "./fl_meta_test"
RESULTS_DIR = "./fl_results_evaluation"
TEMP_DIR = "./temp_predictions" # Temp folder for streaming
SPLIT_JSON_PATH = "./splits_federated/federated_split.json"

if not os.path.exists(RESULTS_DIR): os.makedirs(RESULTS_DIR)
if not os.path.exists(TEMP_DIR): os.makedirs(TEMP_DIR)

MODEL_NAME = "FedAvg_Baseline"
...
def expected_calibration_error(y_true_cls, y_pred_cls, n_bins=10):
...
# --- METRICS (NLL / ECE / mAP) ---
def evaluate_full_metrics(model, test_gen, anchors, scenarios_map, mc_samples=1):
            ...
            if has_obj:
                ...
                if len(pred_idx) > 0:
                    ...
                    for c in range(NUM_CLASSES):
                        ...
                        sel_idx = tf.image.non_max_suppression(c_boxes, c_scores, 50, IOU_THRESHOLD).numpy()
                        for idx in sel_idx:
                            box = c_boxes[idx]
...
def main():
    generate_expanded_test_manifests(SPLIT_JSON_PATH, META_TEST_DIR, TEST_DATA_ROOT)
    ...
    test_gen = DataLoaderGenerator(TEST_DATA_ROOT, specific_files=all_files, batch_size=BATCH_SIZE)

    if not os.path.exists(WEIGHTS_PATH):
        print(f"❌ Error: No se encuentra el archivo de pesos: {WEIGHTS_PATH}")
        exit(1)
    ...
    # Generate Anchors
    anchor_gen = AnchorBox(input_shape=INPUT_SHAPE)
    anchors = anchor_gen.anchors 

    print(f"🏗️ Building Model & Loading Weights: {WEIGHTS_PATH}")
    ...
    results = evaluate_full_metrics(model, test_gen, anchors, scenarios_map, mc_samples=5)
    
    # 5. SAVE AND REPORT
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    json_path = os.path.join(RESULTS_DIR, f"{MODEL_NAME}_OnePass_{timestamp}.json")
