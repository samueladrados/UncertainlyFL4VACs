...
def calculate_metrics(model, data_generator, num_batches=10, num_samples=5):
    """
    ...
    for i, (images, _) in enumerate(data_generator):
        ...
        for scale_idx, sample_list in mc_preds_per_scale.items():
...
def run_client(client_id, round_num, global_weights_path, manifest_path, data_root, sc_val_size):
    ...
    # 2. Data Generator
    val_gen = DataLoaderGenerator(data_root, specific_files=diverse_files, batch_size=BATCH_SIZE_VALIDATION)
    ...
    u_novelty, _ = calculate_metrics(local_model, val_gen)
    ...
    _, u_noise = calculate_metrics(local_model, val_gen)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sc_val_size", type=int, required=True)
    args = parser.parse_args()
    
    try:
        run_client(args.client, args.round, args.global_path, args.manifest, args.data_root, args.sc_val_size)
    except Exception as e:
        print(f"❌ [Worker Error] Client {args.client} failed: {e}")
        import traceback
        traceback.print_exc()
        exit(1) # Return error code 1 to master